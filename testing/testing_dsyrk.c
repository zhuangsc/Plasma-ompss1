...
int timing_dsyrk(int argc, char **argv)
{
    /* Check for number of arguments*/
    if ( argc != 8){
        USAGE("SYRK", "alpha beta N K LDA LDC bs rep",
              "   - alpha : alpha coefficient\n"
              "   - beta : beta coefficient\n"
              "   - N : number of columns and rows of matrix C and number of row of matrix A\n"
              "   - K : number of columns of matrix A\n"
...
//    PLASMA_dplgsy( (double)0., N, C, LDC, 51 );
	...
	for ( i = 0; i < rep; ++i ) {
		start = gtime();
		PLASMA_dsyrk(PlasmaUpper, PlasmaNoTrans, N, K, alpha, A, LDA, beta, C, LDC);
		end = gtime();
		elapsed += end - start;
	}

	num_threads = omp_get_max_threads();
	dump_info("plasma_dsyrk.log", num_threads, elapsed, rep);
