...
RunTest(int *iparam, float *dparam, real_Double_t *t_) 
{
    ...
    PASTE_CODE_ALLOCATE_MATRIX( Acpy, check, PLASMA_Complex32_t, LDA, N   );
    PASTE_CODE_ALLOCATE_MATRIX( b,    check, PLASMA_Complex32_t, LDB, NRHS);

     /* Initialiaze Data */
    PLASMA_cplrnt( M, N,    A, LDA,  453 );
    PLASMA_cplrnt( M, NRHS, x, LDB, 5673 );
    ...
    START_TIMING();
    PLASMA_cgels( PlasmaNoTrans, M, N, NRHS, A, LDA, T, x, LDB );
    STOP_TIMING();
