    ...
    double *work, int lwork )
{
    ...
    LAPACKE_dlaset( LAPACK_COL_MAJOR, 'L', m-1, n, zzero, zzero, &work[1], ldw );
    LAPACKE_dlacpy( LAPACK_COL_MAJOR, 'U', m, n, AF, lda, work, ldw );

    /* form W = QR */
    LAPACKE_dormqr_work( LAPACK_COL_MAJOR, 'L', 'N', m, n, k,
                         AF, ldaf, tau, work, ldw,
                         &work[n*ldw], lwork-n*ldw );

    //printf( "\n" );
    //printf( "m %d, n %d, k %d\n", m, n, k );
    //CORE_dprint( m, n, A,    lda,  "A"   );
    //CORE_dprint( m, n, AF,   ldaf, "QR" );
    //CORE_dprint( 1, min(m,n), tau, 1, "tau" );
    //CORE_dprint( m, n, work, ldw,  "Q*R" );
    
    /* compare (subtract) j-th column of W to jpvt(j)-th column of A (jpvt is 1-based) */
    for( j = 0; j < n; ++j ) {
        cblas_daxpy( m, (mzone), &A[(jpvt[j]-1)*lda], 1, &work[j*ldw], 1 );
    }
    
    //CORE_dprint( m, n, work, ldw,  "Q*R - AP" );
    
    return LAPACKE_dlange_work( LAPACK_COL_MAJOR, 'O', m, n, work, ldw, rwork );
}
...
RunTest(int *iparam, double *dparam, real_Double_t *t_)
{
    ...
    tau   = (double*) malloc( 2*N   * sizeof(double) );
    ...
    /* Initialize Data */
    PLASMA_dplrnt(M, N, A, LDA, 123456);
    ...
    /* Check the solution */
    if ( info != 0 ) {
        printf( "\nPLASMA_dgeqp3 returned error %d.\n", info );
    }
    else if ( check ) {
        lwork2 = (M*N + N);
        ...
        dparam[IPARAM_ANORM] = LAPACKE_dlange_work( LAPACK_COL_MAJOR, 'F', N, N, Acpy, LDA, rwork );
        ...
        // double time = cWtime();
        // #ifdef COMPLEX
        // info = LAPACKE_dgeqp3_work( LAPACK_COL_MAJOR, N, N, Acpy, LDA, jpvt2, tau2, work, lwork, rwork );
        // #else
        // info = LAPACKE_dgeqp3_work( LAPACK_COL_MAJOR, N, N, Acpy, LDA, jpvt2, tau2, work, lwork );
        // #endif
        // time = cWtime() - time;
        // if ( info != 0 ) {
        ...
        // /* printf( "   %7.3f", time ); */
        //         
        // CORE_dprint( M, N, Acpy, LDA, "QR_L" );
        // CORE_dprint( 1, min(M,N), tau2, 1, "tau_L" );
        
        
        // cblas_daxpy( LDA*N, (mzone), A, 1, Acpy, 1 );
        // dparam[IPARAM_RES] = LAPACKE_dlange_work( LAPACK_COL_MAJOR, 'F', N, N, Acpy, LDA, rwork );
        // 
        // cblas_daxpy( N, (mzone), tau, 1, tau2, 1 );
        // tnorm = cblas_dnrm2( N, tau2, 1 );
        /* printf( "|t|=%.2e\n", tnorm ); */
