...
#define _NAME  "PLASMA_sgetri_Tile"
...
static int check_getri_factorization(PLASMA_desc *descA1, PLASMA_desc *descA2, int *IPIV)
{
    ...
    float *b = (float *)malloc((descA1->m)*sizeof(float));
    float *x = (float *)malloc((descA1->m)*sizeof(float));

    PLASMA_Desc_Create(&descB, b, PlasmaRealFloat, descA1->mb, descA1->nb, descA1->bsiz,
                       descA1->m, 1, 0, 0, descA1->m, 1);
    PLASMA_Desc_Create(&descX, x, PlasmaRealFloat, descA1->mb, descA1->nb, descA1->bsiz,
                       descA1->m, 1, 0, 0, descA1->m, 1);

    PLASMA_splrnt_Tile( descX, 537 );
    PLASMA_slacpy_Tile( PlasmaUpperLower, descX, descB);

    PLASMA_sgetrs_Tile( PlasmaNoTrans, descA2, IPIV, descX );

    Xnorm = PLASMA_slange_Tile(PlasmaInfNorm, descX);
    Anorm = PLASMA_slange_Tile(PlasmaInfNorm, descA1);
    Bnorm = PLASMA_slange_Tile(PlasmaInfNorm, descB);

    PLASMA_sgemm_Tile( PlasmaNoTrans, PlasmaNoTrans,
                       (float)1.,  descA1, descX,
                       (float)-1., descB);

    Rnorm = PLASMA_slange_Tile(PlasmaInfNorm, descB);
...
static int check_getri_inverse(PLASMA_desc *descA1, PLASMA_desc *descA2, int *IPIV, float *dparam )
{
    ...
    float *work = (float *)malloc(descA1->n*descA1->n*sizeof(float));
    float eps = LAPACKE_slamch_work('e');
    PLASMA_desc        *descW;

    PLASMA_Desc_Create(&descW, work, PlasmaRealFloat,  descA1->mb, descA1->nb, descA1->bsiz,
                       descA1->m, descA1->n, 0, 0, descA1->m, descA1->n);

    PLASMA_slaset_Tile( PlasmaUpperLower, (float)0., (float)1., descW);
    PLASMA_sgemm_Tile( PlasmaNoTrans, PlasmaNoTrans,
                       (float)-1., descA2, descA1,
                       (float)1.,  descW);

    Anorm    = PLASMA_slange_Tile(PlasmaInfNorm, descA1);
    Ainvnorm = PLASMA_slange_Tile(PlasmaInfNorm, descA2);
    Rnorm    = PLASMA_slange_Tile(PlasmaInfNorm, descW);
...
RunTest(int *iparam, float *dparam, real_Double_t *t_)
{
    ...
    PASTE_CODE_ALLOCATE_MATRIX_TILE( descA2, check, float, PlasmaRealFloat, LDA, N, N );
    PASTE_CODE_ALLOCATE_MATRIX( piv, 1, int, N, 1 );

    PLASMA_Alloc_Workspace_sgetri_Tile_Async(descA, &descW);
    PLASMA_splrnt_Tile( descA, 3453 );

    if ( check ) {
        PLASMA_slacpy_Tile( PlasmaUpperLower, descA, descA2 );
    }

    /* PLASMA SGETRF / STRTRI / STRSMRV  */
    {
        ...
        if ( ! iparam[IPARAM_ASYNC] ) {
            ...
            START_TIMING();
            PLASMA_sgetrf_Tile_Async(descA, piv, sequence[0], &request[0]);
            PLASMA_Sequence_Wait(sequence[0]);

            PLASMA_strtri_Tile_Async(PlasmaUpper, PlasmaNonUnit, descA, sequence[1], &request[1]);
            PLASMA_Sequence_Wait(sequence[1]);

            PLASMA_strsmrv_Tile_Async(PlasmaRight, PlasmaLower, PlasmaNoTrans, PlasmaUnit,
                                      (float) 1.0, descA, &descW, sequence[2], &request[2]);
            PLASMA_Sequence_Wait(sequence[2]);

            PLASMA_slaswpc_Tile_Async(descA, 1, descA->m, piv, -1, sequence[3], &request[3]);
            ...
            START_TIMING();
            PLASMA_sgetrf_Tile_Async( descA, piv, sequence[0], &request[0]);
            PLASMA_strtri_Tile_Async( PlasmaUpper, PlasmaNonUnit,
                                      descA, sequence[1], &request[1]);
            PLASMA_strsmrv_Tile_Async(PlasmaRight, PlasmaLower, PlasmaNoTrans, PlasmaUnit,
                                      (float) 1.0,
                                      descA, &descW, sequence[2], &request[2]);
            PLASMA_slaswpc_Tile_Async(descA, 1, descA->m, piv, -1,
        ...
        if ( ! iparam[IPARAM_ASYNC] ) {
            ...
            PLASMA_strtri_Tile(PlasmaUpper, PlasmaNonUnit, descA);
            PLASMA_strsmrv_Tile(PlasmaRight, PlasmaLower, PlasmaNoTrans, PlasmaUnit,
                                (float) 1.0, descA, &descW);
            PLASMA_slaswpc_Tile(descA, 1, descA->m, piv, -1);
            STOP_TIMING();
            ...
            START_TIMING();
            PLASMA_sgetrf_Tile_Async(descA, piv, sequence, &request[0]);
            PLASMA_sgetri_Tile_Async(descA, piv, &descW, sequence, &request[1]);
