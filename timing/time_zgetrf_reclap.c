...
RunTest(int *iparam, double *dparam, real_Double_t *t_) 
{
    ...
    /* Allocate Data */
    PASTE_CODE_ALLOCATE_MATRIX( A, 1, PLASMA_Complex64_t, LDA, NRHS );
    PASTE_CODE_ALLOCATE_MATRIX( ipiv, 1, int, max(M, NRHS), 1 );

    /* Initialiaze Data */
    PLASMA_zplrnt(M, NRHS, A, LDA, 3456);

    /* Save A in lapack layout for check */
    PASTE_CODE_ALLOCATE_COPY(   A2,  check, PLASMA_Complex64_t, A, LDA, NRHS );
