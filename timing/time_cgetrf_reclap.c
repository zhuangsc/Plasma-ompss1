...
RunTest(int *iparam, float *dparam, real_Double_t *t_) 
{
    ...
    /* Allocate Data */
    PASTE_CODE_ALLOCATE_MATRIX( A, 1, PLASMA_Complex32_t, LDA, NRHS );
    PASTE_CODE_ALLOCATE_MATRIX( ipiv, 1, int, max(M, NRHS), 1 );

    /* Initialiaze Data */
    PLASMA_cplrnt(M, NRHS, A, LDA, 3456);

    /* Save A in lapack layout for check */
    PASTE_CODE_ALLOCATE_COPY(   A2,  check, PLASMA_Complex32_t, A, LDA, NRHS );
