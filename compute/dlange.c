 ...
 *              (
 *              ( norm2(A),         NORM = PlasmaSecondNorm
 *
 *  where norm1 denotes the one norm of a matrix (maximum column sum),
 *  normI denotes the infinity norm of a matrix (maximum row sum) and
 *  normF denotes the Frobenius norm of a matrix (square root of sum
 *  of squares). norm2 denotes an estimation of the second norm of a matrix 
 *
 *  Note that max(abs(A(i,j))) is not a consistent matrix
 *  norm.
 *
 *******************************************************************************
 *
                             ...
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    */
    if ( (norm != PlasmaMaxNorm) && (norm != PlasmaOneNorm)
         && (norm != PlasmaInfNorm) && (norm != PlasmaFrobeniusNorm) && (norm != PlasmaSecondNorm) ) {
        plasma_error("PLASMA_dlange", "illegal value of norm");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    /* Quick return */
