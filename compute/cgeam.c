 ...
 *
 *  PLASMA_cgeam - Performs one of the matrix-matrix operations
 *
 ...
 *
 ******************************************************************************/
int PLASMA_cgeam(PLASMA_enum transA, PLASMA_enum transB, int M, int N,
                 PLASMA_Complex32_t alpha, PLASMA_Complex32_t *A, int LDA, PLASMA_Complex32_t beta,  PLASMA_Complex32_t *B, int LDB)
{
    int NB;
    int Am, An, Bm, Bn;
    int status;
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgeam", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_cgeam", "illegal value of transA");
        return -1;
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_cgeam", "illegal value of transB");
        return -2;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_cgeam", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_cgeam", "illegal value of N");
        return -4;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_cgeam", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, Bm)) {
        plasma_error("PLASMA_cgeam", "illegal value of LDB");
        return -10;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgeam", "plasma_tune() failed");
        return status;
    }
 ...
 *
 *  PLASMA_cgeam_Tile - Performs matrix summation.
 *  Tile equivalent of PLASMA_cgeam().
 ...
 *
 ******************************************************************************/
int PLASMA_cgeam_Tile(PLASMA_enum transA, PLASMA_enum transB,
                      PLASMA_Complex32_t alpha, PLASMA_desc *A, PLASMA_Complex32_t beta, PLASMA_desc *B)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    PLASMA_request request = PLASMA_REQUEST_INITIALIZER;
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_cgeam_Tile_Async(transA, transB, alpha, A, beta, B, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_cgeam_Tile_Async - Performs matrix summation.
 *  Non-blocking equivalent of PLASMA_cgeam_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_cgeam_Tile_Async(PLASMA_enum transA, PLASMA_enum transB,
                            PLASMA_Complex32_t alpha, PLASMA_desc *A, PLASMA_Complex32_t beta, PLASMA_desc *B,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgeam_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_cgeam_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_cgeam_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgeam_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgeam_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_cgeam_Tile_Async", "illegal value of transA");
        return plasma_request_fail(sequence, request, -1);
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_cgeam_Tile_Async", "illegal value of transB");
    ...
    if ( (Amb != descB.mb) || (Anb != descB.nb) ) {
        plasma_error("PLASMA_cgeam_Tile_Async", "tile sizes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Ai != descB.i) || (Aj != Bi) || (Bj != descA.j) ) {
        plasma_error("PLASMA_cgeam_Tile_Async", "start indexes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    if (M == 0 || N == 0 ||
        ((alpha == (PLASMA_Complex32_t)0.0 ) && beta == (PLASMA_Complex32_t)1.0))
        return PLASMA_SUCCESS;
    plasma_parallel_call_9(plasma_pcgeam,
        PLASMA_enum, transA,
        PLASMA_enum, transB,
