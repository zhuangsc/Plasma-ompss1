 ...
 * @ingroup double
 *
 *  PLASMA_dgeam - Performs one of the matrix-matrix operations
 *
 ...
 *
 ******************************************************************************/
int PLASMA_dgeam(PLASMA_enum transA, PLASMA_enum transB, int M, int N,
                 double alpha, double *A, int LDA, double beta,  double *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dgeam", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_dgeam", "illegal value of transA");
        return -1;
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_dgeam", "illegal value of transB");
        return -2;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_dgeam", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_dgeam", "illegal value of N");
        return -4;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_dgeam", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, Bm)) {
        plasma_error("PLASMA_dgeam", "illegal value of LDB");
        return -10;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgeam", "plasma_tune() failed");
        return status;
    }
 ...
 * @ingroup double_Tile
 *
 *  PLASMA_dgeam_Tile - Performs matrix summation.
 *  Tile equivalent of PLASMA_dgeam().
 ...
 *
 ******************************************************************************/
int PLASMA_dgeam_Tile(PLASMA_enum transA, PLASMA_enum transB,
                      double alpha, PLASMA_desc *A, double beta, PLASMA_desc *B)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    PLASMA_request request = PLASMA_REQUEST_INITIALIZER;
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dgeam_Tile_Async(transA, transB, alpha, A, beta, B, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_dgeam_Tile_Async - Performs matrix summation.
 *  Non-blocking equivalent of PLASMA_dgeam_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_dgeam_Tile_Async(PLASMA_enum transA, PLASMA_enum transB,
                            double alpha, PLASMA_desc *A, double beta, PLASMA_desc *B,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dgeam_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_dgeam_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_dgeam_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgeam_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgeam_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_dgeam_Tile_Async", "illegal value of transA");
        return plasma_request_fail(sequence, request, -1);
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_dgeam_Tile_Async", "illegal value of transB");
    ...
    if ( (Amb != descB.mb) || (Anb != descB.nb) ) {
        plasma_error("PLASMA_dgeam_Tile_Async", "tile sizes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
/*
    if ( (Ai != descB.i) || (Aj != Bi) || (Bj != descA.j) ) {
        plasma_error("PLASMA_dgeam_Tile_Async", "start indexes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
*/
    ...
    if (M == 0 || N == 0 ||
        ((alpha == (double)0.0 ) && beta == (double)1.0))
        return PLASMA_SUCCESS;
    plasma_parallel_call_8(plasma_pdgeam,
        PLASMA_enum, transA,
        PLASMA_enum, transB,
        double, alpha,
        PLASMA_desc, descA,
