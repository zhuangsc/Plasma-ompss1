 ...
 *
 *  PLASMA_zgeam - Performs one of the matrix-matrix operations
 *
 ...
 *******************************************************************************
 *
 * @sa PLASMA_zgeam_Tile
 * @sa PLASMA_cgeam
 * @sa PLASMA_dgeam
 * @sa PLASMA_sgeam
 *
 ******************************************************************************/
int PLASMA_zgeam(PLASMA_enum transA, PLASMA_enum transB, int M, int N,
                 PLASMA_Complex64_t alpha, PLASMA_Complex64_t *A, int LDA, PLASMA_Complex64_t beta,  PLASMA_Complex64_t *B, int LDB)
{
    int NB;
    int Am, An, Bm, Bn;
    int status;
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_zgeam", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_zgeam", "illegal value of transA");
        return -1;
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_zgeam", "illegal value of transB");
        return -2;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_zgeam", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_zgeam", "illegal value of N");
        return -4;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_zgeam", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, Bm)) {
        plasma_error("PLASMA_zgeam", "illegal value of LDB");
        return -10;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgeam", "plasma_tune() failed");
        return status;
    }
 ...
 *
 *  PLASMA_zgeam_Tile - Performs matrix summation.
 *  Tile equivalent of PLASMA_zgeam().
 ...
 *
 * @sa PLASMA_zgeam
 * @sa PLASMA_zgeam_Tile_Async
 * @sa PLASMA_cgeam_Tile
 * @sa PLASMA_dgeam_Tile
 * @sa PLASMA_sgeam_Tile
 *
 ******************************************************************************/
int PLASMA_zgeam_Tile(PLASMA_enum transA, PLASMA_enum transB,
                      PLASMA_Complex64_t alpha, PLASMA_desc *A, PLASMA_Complex64_t beta, PLASMA_desc *B)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    PLASMA_request request = PLASMA_REQUEST_INITIALIZER;
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_zgeam_Tile_Async(transA, transB, alpha, A, beta, B, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_zgeam_Tile_Async - Performs matrix summation.
 *  Non-blocking equivalent of PLASMA_zgeam_Tile().
 ...
 * @sa PLASMA_dgeam_Tile_Async
 * @sa PLASMA_sgeam_Tile_Async
 *
 ******************************************************************************/
int PLASMA_zgeam_Tile_Async(PLASMA_enum transA, PLASMA_enum transB,
                            PLASMA_Complex64_t alpha, PLASMA_desc *A, PLASMA_Complex64_t beta, PLASMA_desc *B,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_zgeam_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_zgeam_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_zgeam_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgeam_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgeam_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_zgeam_Tile_Async", "illegal value of transA");
        return plasma_request_fail(sequence, request, -1);
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_zgeam_Tile_Async", "illegal value of transB");
    ...
    if ( (Amb != descB.mb) || (Anb != descB.nb) ) {
        plasma_error("PLASMA_zgeam_Tile_Async", "tile sizes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Ai != descB.i) || (Aj != Bi) || (Bj != descA.j) ) {
        plasma_error("PLASMA_zgeam_Tile_Async", "start indexes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    if (M == 0 || N == 0 ||
        ((alpha == (PLASMA_Complex64_t)0.0 ) && beta == (PLASMA_Complex64_t)1.0))
        return PLASMA_SUCCESS;
    plasma_parallel_call_9(plasma_pzgeam,
        PLASMA_enum, transA,
        PLASMA_enum, transB,
