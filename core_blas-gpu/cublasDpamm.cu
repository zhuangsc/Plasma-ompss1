...
static inline int cublasDpamm_a2(cublasHandle_t handle, cublasSideMode_t side, cublasOperation_t trans, cublasFillMode_t uplo,
...
static inline int cublasDpamm_w(cublasHandle_t handle, cublasSideMode_t side, cublasOperation_t trans, cublasFillMode_t uplo,
                               int M, int N, int K, int L,
                               int vi2, int vi3,
 ...
 ******************************************************************************/
int
cublasDpamm(cublasHandle_t handle, int op, cublasSideMode_t side, PLASMA_enum storev,
           int M, int N, int K, int L,
           const double *A1, int LDA1,
                 double *A2, int LDA2,
           const double *V, int LDV,
                 double *W, int LDW)
{
    ...
    if (storev == PlasmaColumnwise) {
        ...
        if (side == CUBLAS_SIDE_LEFT) {
            trans = op == PlasmaA2 ? CUBLAS_OP_N : CUBLAS_OP_T;
            vi2 = trans == CUBLAS_OP_N ? M - L : K - L;
        }
        else {
            trans = op == PlasmaW ? CUBLAS_OP_N : CUBLAS_OP_T;
            vi2 = trans == CUBLAS_OP_N ? K - L : N - L;
        }
        vi3 = LDV * L;
    }
    ...
    /* Rowwise */
        ...
        if (side == CUBLAS_SIDE_LEFT) {
            trans = op == PlasmaW ? CUBLAS_OP_N : CUBLAS_OP_T;
            vi2 = trans == CUBLAS_OP_N ? K - L : M - L;
        }
        else {
            trans = op == PlasmaA2 ? CUBLAS_OP_N : CUBLAS_OP_T;
            vi2 = trans == CUBLAS_OP_N ? N - L : K - L;
        }
        vi2 *= LDV;
        vi3  = L;
    }
    ...
    if (op==PlasmaW) {
        info = cublasDpamm_w(
                handle, side, trans, uplo, M, N, K, L, vi2, vi3,
...
/***************************************************************************/
static inline int
cublasDpamm_w(cublasHandle_t handle, cublasSideMode_t side, cublasOperation_t trans, cublasFillMode_t uplo,
             int M, int N, int K, int L,
             int vi2, int vi3,
             const double *A1, int LDA1,
                   double *A2, int LDA2,
                   ...
                   double *W, int LDW)
{
    ...
    */
    ...
    if (side == CUBLAS_SIDE_LEFT) {
        ...
        if (((trans == CUBLAS_OP_T) && (uplo == CUBLAS_FILL_MODE_UPPER)) ||
            ((trans == CUBLAS_OP_N) && (uplo == CUBLAS_FILL_MODE_LOWER))) {
            ...
            status = cudaMemcpy2D(
                W, LDW*sizeof(double),
                &A2[K-L], LDA2*sizeof(double),
                L*sizeof(double), N, cudaMemcpyDeviceToDevice );
	        ...
	        cublasDtrmm(
                     handle, side, uplo, 
                     trans, CUBLAS_DIAG_NON_UNIT, L, N, 
                     &zone, &V[vi2], LDV, W, LDW, W, LDW);
                //cblas_dtrmm(
        ...
        if (((trans == CUBLAS_OP_T) && (uplo == CUBLAS_FILL_MODE_UPPER)) ||
            ...
            if (L > 0) {
                ...
                status = cudaMemcpy2D(
                    W, LDW*sizeof(double),
                    &A2[LDA2*(K-L)], LDA2*sizeof(double),
                    M*sizeof(double), L, cudaMemcpyDeviceToDevice );
                //LAPACKE_dlacpy_work(LAPACK_COL_MAJOR,
                    //lapack_const(PlasmaUpperLower),
                    //M, L,
	        ...
	        cublasDtrmm(
                     handle, side, uplo, 
                     trans, CUBLAS_DIAG_NON_UNIT, M, L, 
                     &zone, &V[vi2], LDV, W, LDW, W, LDW);
                //cblas_dtrmm(
...
/***************************************************************************/
static inline int
cublasDpamm_a2(cublasHandle_t handle, cublasSideMode_t side, cublasOperation_t trans, cublasFillMode_t uplo,
              int M, int N, int K, int L,
              int vi2, int vi3,
                    double *A2, int LDA2,
              const double *V, int LDV,
