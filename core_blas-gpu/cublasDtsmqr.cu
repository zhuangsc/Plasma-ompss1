                ...
                double *WORK, int LDWORK)
{
    ...
    if ( (M2 < 0) ||
         ( (M2 != M1) && (side == CUBLAS_SIDE_RIGHT) ) ){
        coreblas_error(5, "Illegal value of M2");
        return -5;
    }
    if ( (N2 < 0) ||
         ( (N2 != N1) && (side == CUBLAS_SIDE_LEFT) ) ){
        coreblas_error(6, "Illegal value of N2");
        return -6;
    }
    if ((K < 0) ||
        ( (side == CUBLAS_SIDE_LEFT)  && (K > M1) ) ||
        ( (side == CUBLAS_SIDE_RIGHT) && (K > N1) ) ) {
    ...
    if (((side == CUBLAS_SIDE_LEFT)  && (trans != CUBLAS_OP_N))
        || ((side == CUBLAS_SIDE_RIGHT) && (trans == CUBLAS_OP_N))) {
        i1 = 0;
        i3 = IB;
    }
    else {
